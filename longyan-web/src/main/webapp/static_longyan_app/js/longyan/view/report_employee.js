/**
 * 报表-员工视图
 * report_employee view
 **/
define('js/longyan/view/report_employee', [
        'text!js/longyan/template/report_employee.tpl',
    ...
    function(SimpleTpl, RankListItemTpl, AlertUI, HeaderView, InputBox, GenderBox, RoomBox, SelectBox, ThinkBox, ButtonBox, LinkBox, PieBox, LineBox, LocationBox, EmployeeRecordBox, ListBox, GroupInputRecordBox, InfoTipsBar, TabBox, ReportApi) {
        var view_id = '#report-employee-view';
        var form_id = '#report-employee-form';
        ...
        var LayoutView = Backbone.View.extend({
            ...
            render: function() {
                ...
                //隐藏一些内容
                $(form_id).find('.records-box').find('.input-member-amount').hide();
                $(form_id).find('.records-box').find('.input-member-rate').hide();
                $(form_id).find('.records-box').find('.employee-count').hide();
                $(form_id).find('.records-box').find('.uninput-member-amount').find('.label').html('已录住宅');
                ...
                t.tips_bar = new InfoTipsBar({
                    ...
                    Click: function() {
                        router.navigate('report_community_list/' + t.config.id, {
                            trigger: true
                        });
                    }
                });
            ...
            loadData: function() {
                ...
                ReportApi.getEmployeeInputReport (postData, function(data) {
                        ...
                        if (data && data.employee_name && data.employee_name) {
                            t.header_view.setText(data.employee_name);
                        }
                        //顶部仪表盘数据
                        if (data) {
                            var input_member_rate = 0;
                            if (data.employee_input_community_room_amount) {
                                input_member_rate = ((data.employee_member_input_count / data.employee_input_community_room_amount) * 100).toFixed(0);
                            ...
                            var record_data = {
                                input_member_amount: 0,
                                uninput_member_amount: data.employee_member_input_count,
                                input_member_rate: 0,
                                input_community_amount: data.employee_community_input_count,
                                employee_count: 0
                            }
                            t.record_box.setValue(record_data);

                            if (data.employee_community_input_count) {
                                t.tips_bar.setValue(data.employee_community_input_count);
                            }
                        }
