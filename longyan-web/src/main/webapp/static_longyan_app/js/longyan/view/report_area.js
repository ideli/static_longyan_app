 ...
 * report_area view
 **/
define('js/longyan/view/report_area', [
        'text!js/longyan/template/report_area.tpl',
    ...
    function(SimpleTpl, RankListItemTpl, AlertUI, HeaderView, InputBox, GenderBox, RoomBox, SelectBox, ThinkBox, ButtonBox, LinkBox, PieBox, LineBox, LocationBox, EmployeeRecordBox, ListBox, GroupInputRecordBox, InfoTipsBar, TabBox, ReportApi) {
        var view_id = '#report-area-view';
        var form_id = '#report-area-form';
        ...
        var LayoutView = Backbone.View.extend({
            ...
            render: function() {
                ...
                t.$el.find('.employee-count').find('.label').html('区域员工');
                ...
                t.tips_bar = new InfoTipsBar({
                    ...
                    Click: function() {
                        router.navigate('report_mall_list_by_area/' + t.config.id, {
                            trigger: true
                        });
                    }
                });
            ...
            loadData: function() {
                var t = this;
                var postData = {
                    organizationId: t.config.id || 0
                };
                //打开loading
                tipsAlert.openLoading({
                    content: '加载中...'
                });
                ReportApi.getAreaInputReport (postData, function(data) {
                        //关闭loading
                        tipsAlert.close();
                        //success
                        console.log(data);
                        if (data && data.currentData && data.currentData.name) {
                            t.header_view.setText(data.currentData.name);
                        }
                        //顶部仪表盘数据
                        if (data && data.currentData) {
                            var input_member_rate = 0;
                            if (data.currentData.inputCommunityRoomAmount) {
                                input_member_rate = ((data.currentData.inputMemberAmount / data.currentData.inputCommunityRoomAmount) * 100).toFixed(0);
                            }
                            if (input_member_rate && input_member_rate > 100) {
                                input_member_rate = 100;
                            }
                            var _uninput_member_amount = data.currentData.inputCommunityRoomAmount - data.currentData.inputMemberAmount || 0;
                            if (_uninput_member_amount < 0) {
                                _uninput_member_amount = 0;
                            }
                            var record_data = {
                                input_member_amount: data.currentData.inputMemberAmount || 0,
                                uninput_member_amount: _uninput_member_amount,
                                input_member_rate: input_member_rate,
                                input_community_amount: data.currentData.inputCommunityAmount || 0,
                                employee_count: data.currentData.employeeCount || 0
                            }
